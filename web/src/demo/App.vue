<template>
  <div class="app">
    <div class="top-bar">
      ...
      <label class="toggle-label"><input type="checkbox" v-model="showMACD" /> MACD</label>
      <label class="toggle-label"><input type="checkbox" v-model="showVolume" checked /> 成交量</label>
      <span class="signal-count" v-if="signals.length">🎯 {{ signals.length }} 个信号</span>
    </div>
...
async function fetchData() {
  ...
  try {
    // 获取名称
    const nr = await fetch(`/api/stock-name?code=${code}&mode=stock`)
    if (nr.ok) {
      const nd = await nr.json()
      stockName.value = nd.name || ''
    }
  } catch (e) {}
  ...
  try {
    ...
    if (d.klines?.length) {
      klines.value = d.klines
      signals.value = d.signals || []
      signalMarkers.value = buildMarkers(d.signals)
      return
    }
  } catch (e) {}
...
function buildMarkers(signals) {
  ...
  for (const s of signals) {
    if (s.prior_high_date && s.prior_high_price) {
      markers.push({ date: s.prior_high_date, price: s.prior_high_price, color: '#FF9800', symbol: 'circle', size: 10 })
    }
    if (s.bottom_date && s.bottom_price) {
      markers.push({ date: s.bottom_date, price: s.bottom_price, color: '#4CAF50', symbol: 'circle', size: 10 })
    }
  }
  return markers
}
...
.toggle-label { font-size: 0.65rem; color: var(--text-secondary); display: flex; align-items: center; gap: 4px; cursor: pointer; }
